...
class Trainer():
    ...
    def load_cxr_state(self, ckpt_path, prefix="cxr_model.", strip_module=True):
        """
        只把 checkpoint 里以 `cxr_model.` 开头的参数，加载到 self.model.cxr_model。
        会自动去掉 DataParallel 的 'module.' 前缀，并且只加载形状匹配的权重。
        """
        from collections import OrderedDict

        # PyTorch 2.6+ 如遇 weights_only 限制，可按需加 weights_only=False（前提是你信任权重来源）
        ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        state = ckpt.get("state_dict", ckpt)

        # 目标子模块 & 其 state_dict
        target = getattr(self.model, "cxr_model", self.model)
        target_sd = target.state_dict()

        # 可选：去掉 DataParallel 的 'module.' 前缀
        def strip_dp(k):
            return k[7:] if strip_module and k.startswith("module.") else k
        ...
        for k, v in state.items():
            ...
            if not k.startswith(prefix):
                continue
            subk = k[len(prefix):]  # 去掉 'cxr_model.' 前缀，对齐到子模块命名
            if subk in target_sd and target_sd[subk].shape == v.shape:
                picked[subk] = v

        # 实际加载
        incompatible = target.load_state_dict(picked, strict=False)

        # 打印简报
        print(f"✅ CXR preload from: {ckpt_path}")
        print(f"   picked: {len(picked)} | missing: {len(incompatible.missing_keys)} | unexpected: {len(incompatible.unexpected_keys)}")
        return incompatible
