...
#     global diagnosis_labels
#     diagnoses['value'] = 1
#     labels = (diagnoses[['stay_id', 'icd_code', 'value']]
#               .drop_duplicates()
#               .pivot(index='stay_id', columns='icd_code', values='value'))

#     # 一次性补齐缺失列并按 diagnosis_labels 排序，缺的用 0 填
...
#     # 重命名列
#     labels.columns = [f'Diagnosis {d}' for d in diagnosis_labels]
#     return labels
...
def clean_fio2(df):
    ...
    # is_str = np.array(list(map(lambda x: isinstance(x, str), df.value.tolist())), dtype=bool)
