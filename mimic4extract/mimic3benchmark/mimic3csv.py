...
#     df.columns = [c.lower() for c in df.columns]        # 列名统一小写
#     if 'ethnicity' not in df.columns:
#         if 'race' in df.columns:
#             df['ethnicity'] = df['race']                # MIMIC-IV 近似映射
#         else:
#             df['ethnicity'] = np.nan
#     need = ['subject_id','hadm_id','admittime','dischtime','deathtime','ethnicity']
#     missing = [c for c in need if c not in df.columns]
#     if missing:
#         raise KeyError(f"missing columns: {missing} in {path} (MIMIC version mismatch?)")
#     return df[need]
...
def _open_text(path, encoding="utf-8"):
    if path.endswith(".gz"):
        # gzip 文本读取
...
def read_events_table_by_row(mimic3_path, table):
    ...
    table_l = table.lower()
    fpath = os.path.join(mimic3_path, csv_files[table_l])

    # 如果不是 UTF-8，可以把 encoding 改成 'latin1' 或加 errors='replace'
    with _open_text(fpath, encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for i, row in enumerate(reader):
            # MIMIC-IV: chartevents 里有 stay_id；labevents 没有，给个空串以兼容下游
            if 'stay_id' not in row:
                row['stay_id'] = ''
            yield row, i, nb_rows.get(table_l, None)
